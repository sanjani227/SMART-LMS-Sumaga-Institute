...
import { User, Wallet, CheckCircle2, Loader2, ArrowRight, Building, CreditCard, Upload } from "lucide-react";
...
export default function ChildPaymentsContent() {
    ...
    const initializePage = async () => {
        try {
            ...
            const targetId = (requestedId && linkedChildren.some(c => c.studentId === requestedId))
                ? requestedId
    ...
    const submitPayment = async () => {
        ...
        try {
            ...
            if (response.data.code === 200) {
                ...
                setSlipBase64("");
                await fetchPayments(selectedStudentId);
            } else {
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="w-full md:w-auto min-w-[250px]">
            ...
            <select
              ...
              className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-orange-400 focus:outline-none bg-white font-semibold"
            >
              {children.map((child) => (
